...
| --------------------------------------- | ------------------ | ---------------- | ------------------------------------------ | ---------------------------------------- |
| `**/{project_id}/create_log**`          | Yes (`**secret**`) | Node / server    | `**AuraServer**` (`auralogger-cli/server`) | `**server-check**`, `**test-serverlog**` |
...
- `**POST /api/proj_auth`** — secret → project id, session, styles
- `**POST /api/logs**` — filtered log fetch for `**get-logs**`
